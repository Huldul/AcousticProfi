...
<main class="main-page design soundproofing about">
            <div class="soundproofing__img main-page__video">
                <div class="breadcrumbs container">
                    <ul class="breadcrumbs__list">
                        ...
                        <li>•</li>
                        <li>Акустическое проектирование</li>
                    </ul>
                </div>
                <img src="{{asset("storage/".$page->head_image)}}" alt="">
            </div>
            <div class="design-links__group soundproofing-links__group about-links__group container">
                <a href="/soundproofing">Звукоизоляция</a>
                <a href="#">Акустическое проектирование</a>
            </div>
            <section class="main-page__design design__more container">
                <div class="bg-svg">
                <h1 class="design__title about-us__title title">Акустическое проектирование</h1>
            ...
            {!!$page->head_text!!}
            <div class="design__desc soundproofing__desc desc">
                <div class="design-desc__wrapper">
                    <div>
                    ...
                    <div class="container">
                    <div class="design__tabs tabs">
                        <ul>
                            @foreach ($effects as $key => $effect)
                            <li><a @if($key === 0) class="active" @endif href="#{{$effect->title}}">{{$effect->title}}</a></li>
                            @endforeach
                        </ul>
                    </div>
                    @foreach ($effects as $effect)
                    <div id="{{$effect->title}}" class="main-tab tab-voice voice active">
                        <ul class="tabs__list">
                            @foreach ($effect->soundTypes as $key => $type)
                            <li><a @if($key === 0) class="active" @endif href="#{{$type->title}}">{{$type->title}}</a></li>
                            @endforeach
                        </ul>
                        @foreach ($effect->soundTypes as $type)
                        <div id="{{$type->title}}" class="voice__container active">
                            @foreach ($type->sounds as $sound)
                            <div class="voice__box">
                                <audio class="audio-player">
                                    ...
                                    $downloadLink = $data[0]['download_link'];
                                    ?>
                                    <source src="{{asset("storage/".$downloadLink)}}" type="audio/mp3">
                                </audio>
                                <img class="bg-audio" src="{{asset("storage/".$sound->image)}}" alt="">
                                    <img  class="play-audio btn-controls" src="{{asset("/img/play-img.png")}}" alt="">
                                    <img  class="stop-audio btn-controls" src="{{asset("/img/play-img.png")}}" alt="">
                ...
                <div class="design__wrapper soundproofing__wrapper">
                    <div class="design__container soundproofing__container">
                        <div class="bg-svg">
                            <h2 class=" title">Акустическое проектирование</h2>
                            <svg class="animated-svg" width="112.000000" height="75.903931" viewBox="0 0 112 75.9039" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <desc>
                                        Created with Pixso.
                                </desc>
                    ...
                    <a class="soundproofing__box" href="/acoustic-inner/{{$item->slug}}">
